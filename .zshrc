...
PATH="$PATH:$HOME/Library/Python/3.12/bin"
...
######## FUNCTIONS
todo() { vi "$HOME/.notes/$(date +'%Y-%m-%d').todo" }
notes() { vi "$HOME/.notes/$(date +'%Y-%m-%d').note" }
tdone() { vi "$HOME/.notes/$(date +'%Y-%m-%d').done" }
ydo() { less "$HOME/.notes/$(date -v '-1d' +'%Y-%m-%d').todo" || exit 1 }
ynote() { less "$HOME/.notes/$(date -v '-1d' +'%Y-%m-%d').note" || exit 1 }
ydone() { less "$HOME/.notes/$(date -v '-1d' +'%Y-%m-%d').done" || exit 1 }

nu() { mkdir -p src test && touch .gitignore README.md }
######## ALIAS
alias k="kubectl"
...
alias d="docker"
alias waiting4hasan="streamlink https://twitch.tv/hasanabi best"
